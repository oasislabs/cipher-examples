...
use oasis_runtime_sdk_contracts::{types, Config, Genesis, Module};

/// Hello contract code.
static HELLO_CONTRACT_CODE: &[u8] =
    include_bytes!("../target/wasm/wasm32-unknown-unknown/release/vigil.wasm");
...
type Contracts = Module<ContractsConfig>;
        ...
        call: transaction::Call {
            ...
            body: cbor::to_value(types::Call {
                ...
                data: cbor::to_vec(cbor::cbor_map! {
                    "create_secret" => cbor::cbor_map!{
                        "name" => cbor::cbor_text!("test"),
                        "value" => cbor::cbor_bytes!(vec![1, 2, 3]),
                        "revelation_set" => cbor::cbor_text!("anyone"),
                        "revelation_timestamp" => cbor::cbor_int!(0)
                    }
                }),
